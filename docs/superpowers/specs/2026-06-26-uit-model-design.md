...
- **Vision encoder:** a **SINGLE shared** `MaskedSwin(SwinTransformer)` instance = the image encoder `E_i`. It encodes BOTH the full image (ITC/ITM/MLM) and the masked image (MIM) — **weights are shared, one model**, not two encoders (per paper Fig 1b: `I` and `I_masked` both pass through the same `E_i`). It is a subclass of `models/swin_transformer.py` (NOT modified); override `forward(x, mask=None)`: `mask=None` ⇒ identical to parent; `mask` given ⇒ replace masked patch tokens with a learnable `mask_token` right after `patch_embed`. Only `mim_decoder` is MIM-specific.
